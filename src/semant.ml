...
and exp the_state = function 
  ...
  | Lambda(vars, body) ->  (* TODO can have partials too *)
      let rec dups = function (* check duplicate argument names *)
        | [] -> ()
        | (Bind(n1, _) :: Bind(n2, _) :: _) when n1 = n2 -> 
            raise (Failure ("SSyntaxError: duplicate argument '" ^ n1 ^ "' in definition of lambda function"))
        | _ :: t -> dups t
      in let _ = dups (List.sort (fun (Bind(a, _)) (Bind(b, _)) -> compare a b) vars) in 
      let the_state = change_state the_state S_func in

      let (map, binds) = List.fold_left 
        (fun (map, out) x -> 
          let (map', name, inferred_t, explicit_t) = assign map (Dyn, (SNoexpr, Dyn), None) x in 
          (map', (Bind(name, explicit_t)) :: out)
        ) (StringMap.empty, []) vars in
...
(* 
  | Pipe(a, b) -> 
      let (t1, e1, _) = expr the_state a in
      exp the_state (Appl(b, a)) *)

  | Noexpr -> (Dyn, SNoexpr, None)
